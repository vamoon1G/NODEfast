...
// Подключение bodyParser для обработки JSON-тел запросов
app.use(express.json());

// Обслуживание статических файлов из папки 'public'
app.use(express.static('public'));

// Обработка POST-запроса от HTML-формы
app.post('/process-urls', async (req, res) => {
    const { urls } = req.body; // Получаем массив URL из тела запроса
    console.log(urls); // Логируем для проверки
    // Здесь вызываем вашу функцию обработки URL
    try {
        await processUrlsAndWriteToExcel(urls); // Предполагается, что эта функция у вас уже реализована
